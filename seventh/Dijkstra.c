...
#include<stdlib.h>
#include<string.h>
#define INFINITY INT_MAX//最大值，在有向图中表示没有边
#define MAX_VERTEX_NUM 20
typedef enum{DG = 1,DN,UDG,UDN} GraphKind;
//有向图，有向网，无向图，无向网
typedef int VRType;
//顶点类型 无权是1/0表示是否相邻
//网是有权值，不相邻是INT_MAX,默认总权值为100%,省略%
...
*/
{
    ...
    if(state1)
    {
        ...
        if(graph-> kind == DN ||graph -> kind == DG)
        {
            for ( ver2 = 1; ver2 < MAX_VERTEX_NUM; ver2 ++)
            {
                if(graph ->arcs[ver1][ver2].adj && graph ->arcs[ver1][ver2].adj  != INT_MAX)//j为始点的
                {
                    DeleteArc(graph,ver_value,graph -> Vertexes[ver2]);
                }
                if(graph ->arcs[ver1][ver2].adj && graph ->arcs[ver1][ver2].adj  != INT_MAX)//j为终点的
                {
...
//这个是外函数，负责做是否访问的数组
{
    ...
    for( i = 1 ; i <= graph .VexNum ; i++)
    {
        ...
        else
        {
            ...
            printf("从v0到v%d上有:v0",graph.Vertexes[i]);
            for(j = 1; j <= graph . VexNum; j++)
            {
...
int main()
{
    ...
    PathMatrix = (int **) calloc(graph -> VexNum + 1,sizeof(int*));// 0号不用和graph->arcs一样
